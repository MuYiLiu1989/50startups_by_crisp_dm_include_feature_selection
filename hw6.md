...
![RMSE vs 特徵數對比](rmse_comparison.png)

**圖表解讀**：
- 水平軸表示模型中使用的特徵數（1-5個）
- 垂直軸表示均方根誤差（RMSE），數值越低表示預測誤差越小
- 10條曲線代表10個不同的特徵選擇方法
- **關鍵觀察**：所有方法在1個特徵時RMSE最低（約7,714），之後隨著特徵數增加而上升，說明只有主特徵（R&D Spend）有益，額外特徵引入噪聲
...
![R² Score vs 特徵數對比](r2_comparison.png)

**圖表解讀**：
- 水平軸表示模型中使用的特徵數（1-5個）
- 垂直軸表示決定係數（R² Score），範圍0-1，數值越高表示模型解釋力越強
- 10條曲線代表10個不同的特徵選擇方法
- **關鍵觀察**：
  - 1個特徵（R&D Spend）達到最高R²（0.9265 ≈ 92.65%的變異解釋率）
  - 從1個增至2個特徵：R²下降至0.9168（下降0.97%）
  - 繼續增加特徵導致R²持續下降至0.8987，證明過度擬合現象
  - 這與RMSE的趨勢完全對應，驗證了結論的穩健性
